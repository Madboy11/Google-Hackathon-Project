...
export default function OraclePanel() {
  ...
  useEffect(() => {
    const poll = async () => {
      try {
        const r = await fetch(`${import.meta.env.VITE_API_URL || ''}/orch/health`);
        if (r.ok) {
  ...
  const submitQuery = async (query: string) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || ''}/orch/agent/stream?query=${encodeURIComponent(query)}`);
